...
export default function Viniles({ setProducto, setIndicePro }) {
    ...
    const jalarBanner = () => {
        var url = 'https://comercializadora.apexmaicol.online/items/readVL.php'
        fetch(url)
            .then((res) => res.json())
            .then(res => {
                setDatosBanner(res)
    ...
    return (
        ...
        <div className='bg-comercia-two h-full lg:h-screen lg:grid lg:grid-cols-7 text-center 2xl:grid-rows-2'>
            ...
            <header className='text-center lg:w-3/12 py-4 flex flex-col md:grid md:grid-cols-5 lg:flex lg:fixed h-full lg:pl-2 lg:justify-between'>
                ...
                <div className='md:col-span-5 lg:flex lg:text-start md:pb-8 p-2 2xl:justify-center'>
                    <h1 className='font-monse text-4xl lg:text-4xl  2xl:text-6xl xl:text-5xl lg:text-center text-white font-black'><i>VINILES Y LAMINADOS </i></h1>
                </div>
...
{/* <button onClick={() => jalarPrecios(i.id
                        )} key={i.id} className='m-4 pb-1 bg-white flex flex-col items-center'>
                            <img src={"https://admincomercial.apexmaicol.online/imagenes/" + i.imagen} alt="" />
                            <div className='mt-1'>
                                <h1 className='font-monse font-bold text-2xl md:text-[1.7rem] xl:text-lg 2xl:text-2xl'>{i.nombre}</h1>
                                <span className='font-monse font-bold text-6xl md:text-6xl xl:text-5xl text-comercia-one'>$ {i.precio}</span>
                                <span className='block font-monse  text-comercia-one text-xs md:text-lg xl:text-sm'>{i.size}</span>
                            </div>
