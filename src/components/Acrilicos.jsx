...
export default function Acrilicos({ setProducto, setIndicePro }) {
    ...
    const jalarBanner = () => {
        var url = 'https://comercializadora.apexmaicol.online/items/readAPFL.php'
        fetch(url)
            .then((res) => res.json())
            .then(res => {
                setDatosBanner(res)
    ...
    return (
        ...
        <div className='bg-comercia-three h-full lg:h-screen lg:grid lg:grid-cols-7 text-center 2xl:grid-rows-2'>
            ...
            <header className='text-center lg:w-3/12 py-4 flex flex-col md:grid md:grid-cols-5 lg:flex lg:fixed h-full lg:pl-2 lg:justify-between'>
                ...
                <div className='md:col-span-5 lg:flex lg:text-start md:pb-8 p-2 2xl:justify-center'>
                    <h1 className='font-monse text-4xl lg:text-2xl  2xl:text-6xl xl:text-5xl lg:text-center text-white font-black'><i>Acrílicos - PVC - FOAM - Leds</i></h1>
                </div>
                    ...
                    datosBanner.map(i => (
                        <Link to={'/producto'} key={i.id} onClick={() => { jalarKey(i) }} className='m-4 pb-1 bg-white flex flex-col items-center'>
                            ...
                            <div className='mt-1'>
                                ...
                                <span className='block font-monse  text-comercia-one text-xs md:text-mg xl:text-sm'>{i.size}</span>
                            </div>
                        </Link>
                    ))
                }
